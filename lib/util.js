...
module.exports.getFullDateNoSepa = function(date){
  ...
  return '' + yy + mm + dd + date.toLocaleTimeString().replace(/:/g,'');
};
...
module.exports.getFileListAndSize = function(dir, callback){
  fs.readdir(dir, function(err, files){
    ...
    files.forEach(function(name){
      var size = fs.statSync(dir + "/" + name).size;
      file_list.push({name: name, size:size});
      all_size += size;
    });
    callback({all_size: all_size, files: file_list});
  });
};
