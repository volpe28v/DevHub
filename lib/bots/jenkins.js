...
module.exports.action = function(data, callback){
  if (data.name.match(/^jenkins$/i)){ return; }
  if (data.msg.match(/Jenkins/i)){  //自分の名前が呼ばれたら
    // Jenkins の設定
    var jenkins_options = util.getSettingFromJsonFile('jenkins');
...
function doJenkins(jenkins_options, data, callback){
  ...
  http.get(get_jobs_options, function(res) {
    if (res.statusCode != 200){
      var reply = {
        name: "Jenkins",
        msg: "@" + data.name + "さん Jenkinsサーバの設定が不正です",
        interval: 1
      };

      callback(reply);
      console.log("status is bad: " + res.statusCode);
      return;
    }
    ...
    res.on('data', function(chunk){
      ...
      if (build_job_count == 0 && (data.msg.match("ls") || data.msg.match("list") || data.msg.match("jobs"))){
        var reply = {
          name: "Jenkins",
