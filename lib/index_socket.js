...
module.exports.setup = function(io){
  io.sockets.on('connection', function(client) {
    ...
    function load_latest_text_with_callback(number, callback){
      text_log.get_tab_numbers(function(data){
        client.emit('memo_tab_numbers',data);

        if (data != null && typeof data.numbers == 'string'){ // コンマ区切りの過去バージョンだった場合の対処
          data.numbers = data.numbers.split(',');
        }

        for (var i = 0; i < number; i++){
          var no = 0;
          if (data != null && data.numbers[i] != null){
            no = Number(data.numbers[i]);
          }else{
            no = i + 1;
          }
    ...
    text_log.get_active_number(function(number){
      ...
      load_latest_text_with_callback(
        number.num,
        function(event_name, arg){
          client.emit(event_name, arg);
        });
    });
    ...
    client.on('memo_number', function(data) {
      ...
      load_latest_text_with_callback(
        ...
        function(event_name, arg){
          client.emit(event_name, arg);
          client.broadcast.emit(event_name, arg);
        });
