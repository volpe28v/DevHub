...
module.exports.setup = function(io){
  io.sockets.on('connection', function(client) {
    ...
    function load_latest_log(data){
      chat_log.size(data.no, function(size){
        if ( size > 0 ){
          chat_log.get(data.no, function(logs){
            client.emit('latest_log' + data.no ,logs);
          });
        }else{
          client.emit('latest_log' + data.no ,[]);
        }
      });
    }
    ...
    client.on('message', function(data) {
      ...
      if (room_matched = data.msg.match(/^room_name:(.+)/)){
        ...
        client.broadcast.emit('message' + data.room_id, room_name);
    ...
    client.on('load_log_more', function(data) {
      chat_log.get_older(data.room_id, data.id, function(logs){
    ...
    client.on('room_name', function(data) {
      chat_log.get_room_name(data.no, function(room_name){
        client.emit('room_name' + data.no, room_name);
      });
    }),
