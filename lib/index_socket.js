...
module.exports.setup = function(io){
  io.sockets.on('connection', function(client) {
    ...
    Promise.all([chat_log.get_active_number(), chat_log.get_tab_numbers(), chat_log.get_room_names()]).then(function(results){
      var number = results[0];
      var tabNumbers = results[1];
      var rooms = results[2];

      client.emit('chat_number',{
        num: number.num,
        numbers: tabNumbers != null ? tabNumbers.numbers : null,
        rooms: rooms.filter(function(elem){ return elem.no != null; })
      });
    });

    function send_latest_logs(number, tabNumbers, callback){
      var valid_numbers = [];
      ...
      valid_numbers.forEach(function(no){
        text_log.get_latest_by_no(no, function(latest){
          if (latest != null){
            callback('text' + latest.no ,latest);
          }
        });
      });
      ...
      valid_numbers.forEach(function(no){
        text_log.get_logs_by_no(no, function(logs){
          if (logs.length > 0){
            ...
            callback('text_logs' + logs[0].no, logs);
          }
        });
      });
    }
    ...
    Promise.all([text_log.get_active_number(), text_log.get_tab_numbers()]).then(function(results){
      ...
      send_latest_logs(number.num, tabNumbers, function(event_name, event_data){
    ...
    client.on('chat_number', function(data) {
      Promise.all([chat_log.get_tab_numbers(), chat_log.get_room_names()]).then(function(results){
        var tabNumbers = results[0];
        var rooms = results[1];
        ...
        client.broadcast.emit('chat_number', number);
      });

      chat_log.update_active_number(data);
    });

    client.on('chat_tab_numbers', function(data) {
      client.broadcast.emit('chat_tab_numbers', data);
      chat_log.update_tab_numbers(data);
    });
    ...
    client.on('memo_number', function(data) {
      ...
      text_log.get_tab_numbers().then(function(tabNumbers){
        send_latest_logs(data.num, tabNumbers, function(event_name, event_data){
          client.emit(event_name, event_data);
          client.broadcast.emit(event_name, event_data);
        });
      });
