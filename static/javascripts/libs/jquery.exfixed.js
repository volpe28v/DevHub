/*
 * 	exFixed 1.3.0rc1 - jQuery plugin
 *	written by Cyokodog	
 *
...
//		if (c.oldBrowser && !/hidden|scroll/i.test(c.target.css('overflow'))) {
	...
	$.extend($.ex.fixed.prototype, {
		...
		_parseAuto : function( size ){
			var o = this , c = o.config;
			c.autosize = c.autosize || {};
			o._eachSize( function( pa ){
				delete c.autosize[pa.name];
				if(/auto/i.test(size[pa.name])){
					c.autosize[pa.name] = 'auto';
				}
			} );
		},
			...
			if(!c.oldBrowser){
				c.target.css( 
					$.extend(
						...
						{position:opt.unfixed?'absolute':'fixed'},c.autosize
					)
				);
			}
			else{
